...
\begin{tikzpicture}
    ...
    % Nombrar vértices
    \def\offset{0.3}
    \draw[fill=black] ($(center) + (\angleA:\circumradius + \offset)$) node {$A$};
    \draw[fill=black] ($(center) + (\angleB:\circumradius + \offset)$) node {$B$};
    \draw[fill=black] ($(center) + (\angleC:\circumradius + \offset)$) node {$C$};
    \draw[fill=black] ($(center) + (\angleD:\circumradius + \offset)$) node {$D$};
