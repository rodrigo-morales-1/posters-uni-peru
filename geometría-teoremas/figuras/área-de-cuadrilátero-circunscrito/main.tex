...
\ifnum\isTangential=1
    ...
    \draw [name intersections={of=bisc1 and bisc2, by={centerOfIncircle}}]
        ...
        in (centerOfIncircle) circle [radius=\n1] \pgfextra{\xdef\radiusOfIncircle{\n1}};

    % Dibujar radio de la circunferencia
    \def\angle{135}
    \def\offset{0.3}
    \draw[fill=black] (centerOfIncircle) circle (1pt);
    % Punto arbitrario
    \coordinate(P) at ($(centerOfIncircle) + (\angle:\radiusOfIncircle)$);
    \draw (centerOfIncircle) -- (P)  node [midway, above right] {$r$};
