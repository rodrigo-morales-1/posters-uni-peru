...
Para convertir un archivo =*.tex= (extensión de documentos de LaTeX) a un archivo =*.pdf=, es necesario tener instalado un compilador de LaTeX. Una alternativa a instalar un compilador que es más accesible para principiantes es usar un compilador online en tu navegador web (ejm. [[https://es.overleaf.com/][Overleaf]]). Overleaf es gratis, solo es necesario registrarse para poder usar el compilador online.
